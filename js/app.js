...
async function fetchTrending() {
  if (!YTApi.hasKey()) {
    showToast('Set your API key to load trending videos', 'error');
    return;
  }
...
function renderCommandResults(query = '') {
  ...
  html += `
    ...
    <div class="command-item" onclick="openModal('api-key-modal'); closeCommandPalette();">
      <span class="command-item-icon">🔑</span>
      <span class="command-item-label">Set API Key</span>
    </div>
  `;
...
async function refreshData() {
  if (!YTApi.hasKey()) {
    showToast('Set your API key to refresh data', 'warning');
    return;
  }
